...
public class CodeGenerator extends VisitorAdaptor {
    ...
    public void visit(StmtRead fchar) {
        if(fchar.getDesignator().obj.getType().equals(Tab.charType)){
            Code.put(Code.bread);
    ...
    @Override
    public void visit(StmtPrintSize stmt) {
        if(stmt.getExpr().obj.getType().equals(Tab.charType)){
            Code.loadConst(stmt.getSize());
            Code.put(Code.bprint);
        }
        else {
            Code.loadConst(stmt.getSize());
    ...
    private void ord(){
        ...
        Obj arg =ord.getLocalSymbols().stream().findFirst().orElse(null);
        ord.setAdr(Code.pc);
        Code.put(Code.enter);
        Code.put(1);
        Code.put(1);
    ...
    private void len(){
        ...
        Obj arg =len.getLocalSymbols().stream().findFirst().orElse(null);
        len.setAdr(Code.pc);
        Code.put(Code.enter);
        Code.put(1);
        Code.put(1);
    ...
    private void chr(){
        ...
        Obj arg =chr.getLocalSymbols().stream().findFirst().orElse(null);
        chr.setAdr(Code.pc);
        Code.put(Code.enter);
        Code.put(1);
        Code.put(1);
    ...
    @Override
    public void visit(ConstructorName method) {
        currrentMethod=method.obj;
        method.obj.setAdr(Code.pc);
        ...
        Code.put(Code.enter);
        Code.put(((currentType!=null)?1:0));
        Code.put(lvc.getCnt()+((currentType!=null)?1:0));
    }
    ...
    public void visit(ConstructorDecl methodDecl){
        Code.put(Code.exit);
        Code.put(Code.return_);
    }
    ...
    public void visit(FactorObject object){
        ...
        if(vtAddresses.get(object.obj.getName())==null){
            if(newAdvance.get(object.obj.getName())==null){
                //new advance sluzi ako se new pozove pre nego sto je inicijalizovana vt
        ...
        Code.put2(0);
        //pozvati konstruktor
        Obj constructor=object.obj.getType().getMembers().stream().filter(e->{
            return e.getName().startsWith("__") && !e.getName().equals("__VTP");
